...
def test_valid_ipv4_1():
    result = runner.invoke(app, ["tests/test_files/ipv4.log", "--ipv4", "127.0.0.1"])
...
def test_valid_ipv4_3():
    result = runner.invoke(
        app, ["tests/test_files/ipv4.log", "--ipv4", "192.168.1.255"]
    )
    assert result.exit_code == 0
    print(result.stdout)
...
def test_invalid_ipv4_1():
    result = runner.invoke(
        app, ["tests/test_files/ipv4.log", "--ipv4", "30.168.1.255.1"]
    )
...
def test_invalid_ipv4_3():
    result = runner.invoke(
        app, ["tests/test_files/ipv4.log", "--ipv4", "192.168.1.256"]
    )
    assert result.exit_code == 2
    print(result.stdout)
    assert "Error: Invalid value" in result.stdout
